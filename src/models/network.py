...
class FlowVelocityNet(nn.Module):
    def __init__(
        ...
        self.pair_mlp = make_mlp(3 * STATE_DIM + 3 + time_dim, hidden_dim, hidden_dim, num_layers)
        self.node_mlp = make_mlp(STATE_DIM + 1 + time_dim + hidden_dim, hidden_dim, STATE_DIM, num_layers)
    ...
    def forward(
        ...
        pair_features = torch.cat(
            [x_i, x_j, x_j - x_i, r_i, r_j, r_i + r_j, tau_pair],
            dim=-1,
        )
        ...
        node_features = torch.cat([x, radius.unsqueeze(-1), tau_node, messages], dim=-1)
